...
class Booking extends BMS_Controller
{
	...
	function step2(){
		...
		if($this->session->userdata('cust_id') != NULL) {
			...
			if($this->form_validation->run() && ((strtotime($this->input->post('date_check_out')) - strtotime($this->input->post('date_check_in')))/3600/24) > 0 && strtotime($this->input->post('date_check_in')) >= date("m/d/Y")){
